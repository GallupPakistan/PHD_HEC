...
from config.settings import PASSOUT_DATA_FILE

YEAR_ORDER = [
    "2016-17", "2017-18", "2018-19", "2019-20", "2020-21",
    "2021-22", "2022-23", "2023-24 (Provisional)", "2024-25 (Provisional)",
]
...
def load_gender_wise():
    return pd.read_excel(PASSOUT_DATA_FILE, "GenderWise")


@st.cache_data
def load_level_wise():
    return pd.read_excel(PASSOUT_DATA_FILE, "LevelWise")
...
def gender_totals(gender_agg):
    """Female/Male grand totals (across the filtered years), for the donut chart."""
    return pd.DataFrame(
        {
            "Gender": ["Female", "Male"],
            "Count": [int(gender_agg["Female"].sum()), int(gender_agg["Male"].sum())],
        }
    )
